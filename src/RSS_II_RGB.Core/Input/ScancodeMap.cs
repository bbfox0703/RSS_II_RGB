...
/// <summary>
/// Maps a Windows keyboard scan code (Set 1) + extended flag to a Scope II LED
/// render index. Scan codes are physical positions (unlike virtual-key codes,
/// which remap), so the same press always lights the same physical key.
///
...
///
/// Keys with no scan code (Fn, the ROG logo, Pause) map to -1: they are still lit
/// by base effects, just never react.
/// </summary>
public static class ScancodeMap
{
    private readonly record struct Entry(byte Scan, bool Extended, byte KeyId);

    // (scan code, extended, Scope II keyId). Standard Set 1 make codes.
    ...
    private static readonly int[] _normal = BuildTable(extended: false);
    private static readonly int[] _extended = BuildTable(extended: true);

    /// <summary>Render index for a (scan code, extended) press, or -1 if none.</summary>
    public static int ToKeyIndex(uint scanCode, bool extended)
    {
        if (scanCode > 0xFF)
        {
            return -1;
    ...
    private static int[] BuildTable(bool extended)
    {
        ...
        foreach (Entry e in _entries)
        {
            ...
            {
                table[e.Scan] = ScopeIILayout.IndexForKeyId(e.KeyId);
            }
        }
        return table;
    }
